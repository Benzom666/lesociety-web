  ...
  return (
    <form className="upload-pics" onSubmit={(e) => { e.preventDefault(); handleSubmit(onSubmit(reduxValues)) }}>
      ...
      <div className="images-uploads">
        ...
        {isImageTouched && (dimensionValid?.height ? <span className="error">Image should be greater than {dimensionValid?.height}*{dimensionValid?.width}</span> : ((!reduxValues?.imageUpload?.length > 0 || !reduxValues?.imageUpload2?.length > 0 || !reduxValues?.imageUpload3?.length > 0 || !reduxValues?.imageUpload4?.length > 0) ?
